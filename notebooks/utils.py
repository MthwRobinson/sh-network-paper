...
def compute_pca(X, n_components=50):
    ...
    pca_df = pd.DataFrame(
        data=pca_matrix, columns=["pc_{}".format(i) for i in range(pca_matrix.shape[1])]
    )
...
def calc_total_effect(
    ...
    effects_data["avg_min_path"] = (
        all_data["avg_min_path"] if not avg_min_path else avg_min_path
    )
    effects_data["gini_coefficient"] = (
        all_data["gini_coefficient"] if not gini_coefficient else gini_coefficient
    )

    effects_data["avg_clusteringXcrowd_pct"] = (
        effects_data["crowd_pct"] * effects_data["avg_clustering"]
    )
    effects_data["avg_min_pathXcrowd_pct"] = (
        effects_data["crowd_pct"] * effects_data["avg_min_path"]
    )
    effects_data["gini_coefficientXcrowd_pct"] = (
        effects_data["crowd_pct"] * effects_data["gini_coefficient"]
    )

    effects_data["giniXavg_min_path"] = (
        effects_data["gini_coefficient"] * effects_data["avg_min_path"]
    )
    effects_data["giniXclustering"] = (
        effects_data["gini_coefficient"] * effects_data["avg_clustering"]
    )
    effects_data["clusteringXavg_min_path"] = (
        effects_data["avg_clustering"] * effects_data["avg_min_path"]
    )

    effects_data["giniXavg_min_pathXcrowd_pct"] = (
        effects_data["gini_coefficient"]
        * effects_data["avg_min_path"]
        * effects_data["crowd_pct"]
    )
    effects_data["giniXclusteringXcrowd_pct"] = (
        effects_data["gini_coefficient"]
        * effects_data["avg_clustering"]
        * effects_data["crowd_pct"]
    )
    effects_data["clusteringXavg_min_pathXcrowd_pct"] = (
        effects_data["avg_clustering"]
        * effects_data["avg_min_path"]
        * effects_data["crowd_pct"]
    )
    ...
    crowd_pct_param = params["crowd_pct"] + (
        ...
        + effects_data["clusteringXavg_min_path"]
        * params["clusteringXavg_min_pathXcrowd_pct"]
    )
    ...
    for i in range(100):
        ...
        for field in data:
            if field == "Intercept":
                ...
                if ":" in field:
                    fields = field.split(":")
                    data[field].append(mean_values[fields[0]] * mean_values[fields[1]])
                else:
